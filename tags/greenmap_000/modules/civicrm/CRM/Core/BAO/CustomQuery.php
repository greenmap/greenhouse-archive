    ...
    function where( ) {
        ...
        foreach ( $this->_ids as $id => $values ) {
            ...
            foreach ( $values as $tuple ) {
                ...
                case 'String':
                    $sql = 'LOWER(' . CRM_CORE_BAO_CUSTOMQUERY_PREFIX. $field['id'] . '.char_data) ';
                    ...
                    if ( is_array( $value ) ) {
                        ...
                        foreach ( $value as $k => $v ) {
                            $value[$k] =
                                CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR .
                                $v . 
                                CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR;
                        }
                        ...
                        if ($field['html_type'] == 'CheckBox') {
                            foreach ( $value as $k => $v ) { 
                                $sqlValue[] = "( $sql like '%" . CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR . $k . CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR . "%' ) ";
                            }
                            $this->_where[$grouping][] = implode( ' AND ', $sqlValue );
                            $this->_qill[$grouping][]  = "$field[label] $op $qillValue";
                        } else { // for multi select
                            foreach ( $value as $k => $v ) { 
                                $sqlValue[] = "( $sql like '%" . CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR . $v . CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR . "%' ) ";
                            }
                            $this->_where[$grouping][] = implode( ' AND ', $sqlValue ); 
                            $this->_qill[$grouping][]  = "$field[label] $op $qillValue";
                        }                    
                ...
                case 'Int':
                    if ( $field['is_search_range'] ) {
                        ...
                        $this->_where[$grouping][] = CRM_CORE_BAO_CUSTOMQUERY_PREFIX. $field['id'] . ".int_data {$op} " . CRM_Utils_Type::escape( $value, 'Integer' );
                ...
                case 'Float':
                    if ( $field['is_search_range'] ) {
                        ...
                        $this->_where[$grouping][] = CRM_CORE_BAO_CUSTOMQUERY_PREFIX. $field['id'] . ".float_data {$op} " . CRM_Utils_Type::escape( $value, 'Float' );
                        $this->_qill[$grouping][]  = $field['label'] . " {$op} {$value}";
                    }
                    continue;                    
                ...
                case 'Money':
                    if ( $field['is_search_range'] ) {
                        ...
                        $this->_where[$grouping][] = CRM_CORE_BAO_CUSTOMQUERY_PREFIX. $field['id'] . '.decimal_data = ' . CRM_Utils_Type::escape( $value, 'Float' );
                        $this->_qill[$grouping][]  = $field['label'] . " {$op} {$value}";
                    }
                    continue;
                ...
                case 'Memo':
                    $val = CRM_Utils_Type::escape( strtolower(trim($value)), 'String' );
                    $this->_where[$grouping][] = CRM_CORE_BAO_CUSTOMQUERY_PREFIX. $field['id'] . ".memo_data {$op} '{$val}'";
                    $this->_qill[$grouping][] = "$field[label] $op $value";
                    continue;
                ...
                case 'Date':
                    ...
                    if ( ! $fromValue && ! $toValue ) {
                        ...
                        $this->_where[$grouping][] = CRM_CORE_BAO_CUSTOMQUERY_PREFIX. $field['id'] . ".date_data {$op} {$date}";
                        $date = CRM_Utils_Date::format( $value, '-' ); 
                        ...
                        if ( $fromDate ) {
                            $this->_where[$grouping][] = CRM_CORE_BAO_CUSTOMQUERY_PREFIX. $field['id'] . ".date_data >= $fromDate";
                            $fromDate = CRM_Utils_Date::format( $fromValue, '-' );
                            $this->_qill[$grouping][]  = $field['label'] . ' >= ' .
                                CRM_Utils_Date::customFormat( $fromDate );
                        }
                        if ( $toDate ) {
                            $this->_where[$grouping][] = CRM_CORE_BAO_CUSTOMQUERY_PREFIX. $field['id'] . ".date_data <= $toDate";
                            $toDate = CRM_Utils_Date::format( $toValue, '-' );
                            $this->_qill[$grouping][]  = $field['label'] . ' <= ' .
                ...
                case 'StateProvince':
                    ...
                    if ( $value ) {
                        $this->_where[$grouping][] = CRM_CORE_BAO_CUSTOMQUERY_PREFIX. $field['id'] . ".int_data {$op} " . CRM_Utils_Type::escape( $value, 'Int' );
