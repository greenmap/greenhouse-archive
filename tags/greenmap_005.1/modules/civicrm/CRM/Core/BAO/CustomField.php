...
class CRM_Core_BAO_CustomField extends CRM_Core_DAO_CustomField {
     ...
     */
      function &getFields($contactType = 'Individual', $showAll = false ) {
        if ( ! $GLOBALS['_CRM_CORE_BAO_CUSTOMFIELD']['_importFields'] || ! CRM_Utils_Array::value( $contactType, $GLOBALS['_CRM_CORE_BAO_CUSTOMFIELD']['_importFields'] ) ) { 
            ...
            $crmDAO =& CRM_Core_DAO::executeQuery( $query, $GLOBALS['_CRM_CORE_DAO']['_nullArray'] );
     ...
     */
     function getDisplayValue( $value, $id, &$options ) {
        ...
        case "Select Date":
            ...
            $dao->find(true);
            if ( $dao->date_parts ) {
                $parts = explode(CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR,$dao->date_parts);
                foreach($parts as $v ) {
                    $format = $format." %".$v ;
                }
     ...
     function setProfileDefaults( $customFieldId, $elementName, &$defaults, $contactId = null, $mode = null ) 
    {
        ...
        if (!trim($value) && ($value != 0) && ($mode != CRM_PROFILE_FORM_MODE_SEARCH) ) {
            $value = $customField->default_value;
        }
