...
class CRM_Core_BAO_UFField extends CRM_Core_DAO_UFField 
{
     ...
     * @static
     */
     function getProfileType($ufGroupId) 
    {
        $contactTypes = array ( );
     ...
     * @static
     */
     function checkProfileGroupType( ) 
    {
        ...
        $ufGroup =& CRM_Core_DAO::executeQuery( $query, $GLOBALS['_CRM_CORE_DAO']['_nullArray'] );
        
        $fields = array( );
        while ( $ufGroup->fetch() ) {
            if (CRM_Core_BAO_UFField::getProfileType($ufGroup->id) == 'Individual') {
                $fields['Individual'] += 1;
            } else if (CRM_Core_BAO_UFField::getProfileType($ufGroup->id) == 'Contribution') {
                $fields['Contribution'] += 1;
            } else {
                $fields['Other'] +=1;
            }
