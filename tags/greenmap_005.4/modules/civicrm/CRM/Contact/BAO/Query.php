...
define( 'CRM_CONTACT_BAO_QUERY_MODE_QUEST',4);
define( 'CRM_CONTACT_BAO_QUERY_MODE_MEMBER',8);
define( 'CRM_CONTACT_BAO_QUERY_MODE_ALL',15);
...
$GLOBALS['_CRM_CONTACT_BAO_QUERY']['_relType'] = null;
...
$GLOBALS['_CRM_CONTACT_BAO_QUERY']['special'] =  array( 'contact_type', 'contact_sub_type', 'sort_name', 'display_name' );
$GLOBALS['_CRM_CONTACT_BAO_QUERY']['skipWhere'] =  null;
$GLOBALS['_CRM_CONTACT_BAO_QUERY']['arrayValues'] =  null;
...
class CRM_Contact_BAO_Query {
    ...
    function &getWhereValues( $name, $grouping ) {
     ...
     function &fixWhereValues( $id, &$values, $wildcard = 0 ) {
        ...
        if  ( ! $GLOBALS['_CRM_CONTACT_BAO_QUERY']['skipWhere'] ) {
            $GLOBALS['_CRM_CONTACT_BAO_QUERY']['skipWhere']   = array( 'task', 'radio_ts', 'uf_group_id' );
        }

        if ( in_array( $id, $GLOBALS['_CRM_CONTACT_BAO_QUERY']['skipWhere'] ) || substr( $id, 0, 4 ) == '_qf_' ) {
            return null;
        }
    ...
    function whereClauseSingle( &$values ) {
        ...
        if ( CRM_Core_BAO_CustomField::getKeyID( $values[0] ) ||
             ( substr( $values[0], 0, CRM_CORE_FORM_CB_PREFIX_LEN ) == CRM_CORE_FORM_CB_PREFIX ) ||
     ...
     function fromClause( &$tables , $inner = null, $right = null, $primaryLocation = true, $mode = 1 ) {
        ...
        foreach ( $tables as $name => $value ) {
            ...
            case 'civicrm_relationship':
                if( $GLOBALS['_CRM_CONTACT_BAO_QUERY']['_relType'] == 'a') {
    ...
    function includeContactIDs( ) {
        ...
        $contactIds = array( ); 
        foreach ( $this->_params as $id => $values ) { 
            if ( substr( $values[0], 0, CRM_CORE_FORM_CB_PREFIX_LEN ) == CRM_CORE_FORM_CB_PREFIX ) { 
                $contactIds[] = substr( $values[0], CRM_CORE_FORM_CB_PREFIX_LEN ); 
            } 
        } 
        if ( ! empty( $contactIds ) ) { 
    ...
    function relationship( &$values ) {
        ...
        $GLOBALS['_CRM_CONTACT_BAO_QUERY']['_relType'] = $rel[1];
        if ( $rel[1] == 'a') {
     ...
     */
     function &defaultReturnProperties( $mode = 1 ) {
        ...
        if ( ! isset( $GLOBALS['_CRM_CONTACT_BAO_QUERY']['_defaultReturnProperties'][$mode] ) ) {
            $GLOBALS['_CRM_CONTACT_BAO_QUERY']['_defaultReturnProperties'][$mode] = CRM_Core_Component::defaultReturnProperties( $mode );

            if ( empty( $GLOBALS['_CRM_CONTACT_BAO_QUERY']['_defaultReturnProperties'][$mode] ) ) {
                $GLOBALS['_CRM_CONTACT_BAO_QUERY']['_defaultReturnProperties'][$mode] = array( 
                                                               'home_URL'               => 1, 
                                                               'image_URL'              => 1, 
                                                               'legal_identifier'       => 1, 
        ...
        return $GLOBALS['_CRM_CONTACT_BAO_QUERY']['_defaultReturnProperties'][$mode];
    }
     ...
     */
     function apiQuery( $params = null, $returnProperties = null, $options = null ,$sort = null, $offset = 0, $row_count = 25 ) {
        ...
        $dao =& CRM_Core_DAO::executeQuery( $sql, $GLOBALS['_CRM_CORE_DAO']['_nullArray'] );
    ...
    function searchQuery( $offset = 0, $rowCount = 0, $sort = null, 
        ...
        if ( $count ) {
            return CRM_Core_DAO::singleValueQuery( $query, $GLOBALS['_CRM_CORE_DAO']['_nullArray'] );
        }

        $dao =& CRM_Core_DAO::executeQuery( $query, $GLOBALS['_CRM_CORE_DAO']['_nullArray'] );
        if ( $groupContacts ) {
            $ids = array( );
            while ( $dao->fetch( ) ) {
                $ids[] = $dao->id;
