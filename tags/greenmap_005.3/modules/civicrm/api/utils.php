    ...
    foreach ($params as $key => $value) {
        if ($customFieldID = CRM_Core_BAO_CustomField::getKeyID($key)) {
            ...
            if ( $customFields[$customFieldID][3] == 'CheckBox' || $customFields[$customFieldID][3] =='Multi-Select') {
                ...
                $value = CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR.implode(CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR,$newMulValues).CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR;
...
function _crm_add_formatted_param(&$values, &$params) {
    ...
    if (isset($values['preferred_communication_method'])) {
        ...
        $params['preferred_communication_method'] = CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR.implode( CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR , $comm ).CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR;
        return true;
    }
...
function _crm_validate_formatted_contribution(&$params) {
    ...
    foreach ($params as $key => $value) {
        ...
        case 'contact_id':
            ...
            $dao =& new CRM_Core_DAO();
            $svq = $dao->singleValueQuery("SELECT id FROM civicrm_contact WHERE domain_id = $domainID AND id = $value",$GLOBALS['_CRM_CORE_DAO']['_nullArray']);
            if (!$svq) {
