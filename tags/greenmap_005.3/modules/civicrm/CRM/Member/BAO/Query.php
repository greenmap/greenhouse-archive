...
class CRM_Member_BAO_Query {
     ...
     */
     function select( &$query ) {
        ...
        if ( $query->_mode & CRM_CONTACT_BAO_QUERY_MODE_MEMBER ) {
     ...
     function where( &$query ) {
        foreach ( array_keys( $query->_params ) as $id ) {
            if ( substr( $query->_params[$id][0], 0,7 ) == 'member_' ) {
                CRM_Member_BAO_Query::whereClauseSingle( $query->_params[$id], $query );
            }
        }
    }
     ...
     function from( $name, $mode, $side ) {
        ...
        case 'civicrm_membership_type':
            if ( $mode & CRM_CONTACT_BAO_QUERY_MODE_MEMBER ) {
