...
<?php $resultgallery = db_query("SELECT ndfp.field_photo_alt, n.title, n.nid
								FROM node_content_photo p 
									INNER JOIN node n ON p.vid = n.vid
                  INNER JOIN node_data_field_photo ndfp ON n.vid = ndfp.vid
...
if ($number > 0) { ?>
	...
	foreach ($item as $path) { ?>
		<a href="<?php print $node_url ?>"><?php print theme('imagecache', gallerythumb, $path['value']) ; ?></a>
	<?php  } ?>
	

<?php } else {
	print t('There are no photos in this album yet');
}?>


<?php if (content_format('field_album_description', $field_album_description[0]) > '') : ?>
	<?php print check_markup(content_format('field_album_description', $field_album_description[0])) ?>
<?php endif; ?>
<?php 
		
// end of gallery div with space to fix layout? ?>
&nbsp;	<br /><br />
</div>
...
<p><a href="<?php print base_path() ?>node/add/content_photo?album_nid=<?php print $node->nid ?>">Add a photo to this album</a></p>
