...
class CRM_Core_Config {
     ...
     * @var string
     */
    var $templateCompileDir  = './templates_c/en_US/';
     ...
     * @var string
     */
    var $fatalErrorTemplate = 'CRM/error.tpl';
     ...
     * @var string
     */
    var $legacyEncoding = 'Windows-1252';
     ...
     */
    var $includeEmailInSearch     = 1;
    var $includeAlphabeticalPager = 1;
    var $includeOrderByClause     = 1;
    var $includeDomainID          = 1;
    var $oldInputStyle            = 1;
    ...
    function CRM_Core_Config() {
        ...
        if (defined('CIVICRM_SMARTYDIR')) {
            $this->smartyDir = CRM_Core_Config::addTrailingSlash(CIVICRM_SMARTYDIR);
        }

        if (defined('CIVICRM_PLUGINSDIR')) {
            $this->pluginsDir = CRM_Core_Config::addTrailingSlash(CIVICRM_PLUGINSDIR);
        }

        if (defined('CIVICRM_TEMPLATEDIR')) {
            $this->templateDir = CRM_Core_Config::addTrailingSlash(CIVICRM_TEMPLATEDIR);
        }

        if (defined('CIVICRM_TEMPLATE_COMPILEDIR')) {
            $this->templateCompileDir = CRM_Core_Config::addTrailingSlash(CIVICRM_TEMPLATE_COMPILEDIR);
        ...
        if ( defined( 'CIVICRM_CUSTOM_FILE_UPLOADURL' ) ) {
            $this->customUploadURL = CRM_Core_Config::addTrailingSlash( CIVICRM_CUSTOM_FILE_UPLOADURL, '/' );
        }
        ...
        if ( defined( 'CIVICRM_CUSTOM_FILE_UPLOADDIR' ) ) {
            $this->customFileUploadDir = CRM_Core_Config::addTrailingSlash( CIVICRM_CUSTOM_FILE_UPLOADDIR );
        ...
        if ( defined( 'CIVICRM_LC_MESSAGES' ) ) {
            ...
            $this->templateCompileDir .= CRM_Core_Config::addTrailingSlash($this->lcMessages);
            CRM_Utils_File::createDir( $this->templateCompileDir );
        }
        ...
        if ( defined( 'CIVICRM_MYSQL_PATH' ) ) {
            $this->mysqlPath = CRM_Core_Config::addTrailingSlash( CIVICRM_MYSQL_PATH );
        }
        ...
        if ( defined( 'CIVICRM_CAPTCHA_FONT_PATH' ) ) {
            $this->captchaFontPath = CRM_Core_Config::addTrailingSlash( CIVICRM_CAPTCHA_FONT_PATH );
        }
     ...
     */
     function &getMailer( ) {
        if ( ! isset( $GLOBALS['_CRM_CORE_CONFIG']['_mail'] ) ) {
            if ( $GLOBALS['_CRM_CORE_CONFIG']['_singleton']->smtpServer == '' ||
                 $GLOBALS['_CRM_CORE_CONFIG']['_singleton']->smtpServer == 'YOUR SMTP SERVER' ) {
