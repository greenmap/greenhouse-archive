...
$GLOBALS['_CRM_CORE_DAO']['_nullObject'] =  null;
$GLOBALS['_CRM_CORE_DAO']['_nullArray'] =  array( );
...
$GLOBALS['_CRM_CORE_DAO']['cid'] =  null;
...
class CRM_Core_DAO extends DB_DataObject {
    ...
    function table() {
        ...
        foreach ( $fields as $name => $value ) {
            ...
            if ( CRM_Utils_Array::value( 'required', $value ) ) {
                $table[$value['name']] += CRM_CORE_DAO_DB_DAO_NOTNULL;
            }
        }
    ...
    function log( $created = false ) {
        ...
        if ( ! $GLOBALS['_CRM_CORE_DAO']['cid'] ) {
            $session =& CRM_Core_Session::singleton( );
            $GLOBALS['_CRM_CORE_DAO']['cid'] = $session->get( 'userID' );
        }
        ...
        $dao->modified_id   = $GLOBALS['_CRM_CORE_DAO']['cid'];
     ...
     function makeAttribute( $field ) {
        if ( $field ) {
            if ( $field['type'] == CRM_UTILS_TYPE_T_STRING ) {
                ...
                return $attributes;
            } else if ( $field['type'] == CRM_UTILS_TYPE_T_INT || $field['type'] == CRM_UTILS_TYPE_T_FLOAT || $field['type'] == CRM_UTILS_TYPE_T_MONEY ) {
                $attributes['size']      = 4;
     ...
     * @access public
     */
     function getFieldValue( $daoName, $id, $fieldName = 'name', $idName = 'id' ) {
        require_once(str_replace('_', DIRECTORY_SEPARATOR, $daoName) . ".php");
        eval( '$object   =& new ' . $daoName . '( );' );
        $object->$idName =  $id;
        $object->selectAdd( );
     ...
     * @access public
     */
     function &executeQuery( $query, &$params, $abort = true ) {
        $queryStr = CRM_Core_DAO::composeQuery( $query, $params, $abort );
     ...
     * @access public 
     */ 
     function singleValueQuery( $query, &$params, $abort = true ) {
        $queryStr = CRM_Core_DAO::composeQuery( $query, $params, $abort );
        $dao =& new CRM_Core_DAO( ); 
        $dao->query( $queryStr ); 
