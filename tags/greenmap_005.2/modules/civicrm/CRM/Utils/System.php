...
$GLOBALS['_CRM_UTILS_SYSTEM']['_callbacks'] =  null;
$GLOBALS['_CRM_UTILS_SYSTEM']['version'] = null;
$GLOBALS['_CRM_UTILS_SYSTEM']['pid'] =  null;
...
class CRM_Utils_System {
    ...
    function getModuleSetting($pModuleName,$pSetting) {
     ...
     function memory( $title = null ) {
        ...
        if ( ! $GLOBALS['_CRM_UTILS_SYSTEM']['pid'] ) {
            $GLOBALS['_CRM_UTILS_SYSTEM']['pid'] = posix_getpid( );
        }

        $memory = str_replace("\n", '', shell_exec("ps -p". $GLOBALS['_CRM_UTILS_SYSTEM']['pid'] ."-o rss="));
        $memory .= ", " . time( );
        if ( $title ) {
     ...
     */
     function validCallback( $callback ) {
        if ( $GLOBALS['_CRM_UTILS_SYSTEM']['_callbacks'] === null ) {
            $GLOBALS['_CRM_UTILS_SYSTEM']['_callbacks'] = array( );
        }

        if ( ! array_key_exists( $callback, $GLOBALS['_CRM_UTILS_SYSTEM']['_callbacks'] ) ) {
            if ( strpos( $callback, '::' ) !== false ) {
                list($className, $methodName) = explode('::', $callback);
                $fileName = str_replace('_', DIRECTORY_SEPARATOR, $className) . '.php';
                @include_once( $fileName ); // ignore errors if any
                if ( ! class_exists( $className ) ) {
                    $GLOBALS['_CRM_UTILS_SYSTEM']['_callbacks'][$callback] = false;
                } else {
                    ...
                    if ( ! method_exists( $object, $methodName ) ) {
                        ...
                        $GLOBALS['_CRM_UTILS_SYSTEM']['_callbacks'][$callback] = true;
                    }
                }
            } else {
                $GLOBALS['_CRM_UTILS_SYSTEM']['_callbacks'][$callback] = function_exists( $callback );
            }
        }
        return $GLOBALS['_CRM_UTILS_SYSTEM']['_callbacks'][$callback];
    }
