...
class CRM_Import_Parser_Contact extends CRM_Import_Parser {
    ...
    function import( $onDuplicate, &$values) {
        ...
        if ( $relationship ) {
            ...
            if ( ( CRM_Import_Parser_Contact::isDuplicate($newContact)  || is_a( $newContact,CRM_Contact_BAO_Contact ) ) && $primaryContactId ) {
    ...
    function isErrorInCustomData($params, &$errorMessage) {
        ...
        foreach ($params as $key => $value) {
            if ($customFieldID = CRM_Core_BAO_CustomField::getKeyID($key)) {
                ...
                CRM_Import_Parser_Contact::isErrorInCustomData( $params[$key] ,$errorMessage );
            }
        }
        //return true;
    }
    ...
    function isErrorInCoreData($params, &$errorMessage) {
        foreach ($params as $key => $value) {
            if ( $value ) {
                ...
                case 'deceased_date': 
                    if (! CRM_Utils_Rule::date($value)) {
                        CRM_Import_Parser_Contact::addToErrorMsg('Deceased Date', $errorMessage);
                    }
                    break;
                case 'gender':    
...
//                     break;
                ...
                default : 
                    if (is_array($params[$key]) && $params[$key]["contact_type"]) {
                        ...
                        CRM_Import_Parser_Contact::isErrorInCoreData($params[$key],$errorMessage);
                    }
                }
            }
        }
