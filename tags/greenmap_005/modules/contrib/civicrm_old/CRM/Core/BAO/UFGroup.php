...
class CRM_Core_BAO_UFGroup extends CRM_Core_DAO_UFGroup 
{
     ...
     */
     function getFields( $id, $register = false, $action = null,
        ...
        if ( $group->find( true ) ) {
            ...
            $field =& CRM_Core_DAO::executeQuery( $query, $GLOBALS['_CRM_CORE_DAO']['_nullArray'] );
            ...
            while ( $field->fetch( ) ) {
                if ( $searchable || 
                     ( $field->is_view && $action == CRM_CORE_ACTION_VIEW ) ||
                     ! $field->is_view ) {
     ...
     */
      function getValues( $cid, &$fields, &$values ) {
        ...
        if ( CRM_Core_Permission::access( 'Quest' ) ) {
            ...
            $studentFields = $GLOBALS['_CRM_QUEST_BAO_STUDENT']['multipleSelectFields'];
        }
      ...
      function getModuleUFGroup( $moduleName = null, $count = 0) 
    {
        ...
        $ufGroups = array( );
        while ($dao->fetch( )) {
            if ( $moduleName && $moduleName != 'User Registration' && !CRM_Core_BAO_UFGroup::filterUFGroups($dao->id)) {
                continue;
            }
     ...
     function updateWeight($weight, $ufGroupId) 
    {
        ...
        if ( !$check ) {
            ...
            if ( !empty($fieldIds) ) {
                ...
                CRM_Core_DAO::executeQuery( $sql, $GLOBALS['_CRM_CORE_DAO']['_nullArray'] );
            }
        }
     ...
     * @access public
     */
     function filterUFGroups ($ufGroupId) 
    {
        global $user;
