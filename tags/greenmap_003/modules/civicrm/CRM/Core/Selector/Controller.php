...
define( 'CRM_CORE_SELECTOR_CONTROLLER_PDF',32);
...
class CRM_Core_Selector_Controller {
     ...
     * @access public
     */
    function CRM_Core_Selector_Controller($object, $pageID, $sortID, $action, $store = null, $output = CRM_CORE_SELECTOR_CONTROLLER_TEMPLATE, $prefix = null)
    {
         ...
         */
        $storeRowCount = $store->get( $this->_prefix . CRM_UTILS_PAGER_PAGE_ROWCOUNT );
        if ( $storeRowCount ) {
    ...
    function hasChanged($reset)
    {
         ...
         */
        if ( $reset ||
             $this->_store->get( $this->_prefix . CRM_UTILS_PAGER_PAGE_ID ) == null ||
             $this->_store->get( $this->_prefix . CRM_UTILS_SORT_SORT_ID ) == null ) {
            return false;
        }

        if ( $this->_store->get( $this->_prefix . CRM_UTILS_PAGER_PAGE_ID       ) != $this->_pager->getCurrentPageID       ( ) ||
             $this->_store->get( $this->_prefix . CRM_UTILS_SORT_SORT_ID        ) != $this->_sort->getCurrentSortID        ( ) || 
             $this->_store->get( $this->_prefix . CRM_UTILS_SORT_SORT_DIRECTION ) != $this->_sort->getCurrentSortDirection ( ) ) {
            return true;
        }
        return false;
    }
    ...
    function run( )
    {
        ...
        if ($this->_output == CRM_CORE_SELECTOR_CONTROLLER_EXPORT|| $this->_output == CRM_CORE_SELECTOR_CONTROLLER_SCREEN) {
            ...
            $rows = CRM_Core_Selector_Controller::getRows( $this );
            ...
            if ($this->_output & CRM_CORE_SELECTOR_CONTROLLER_SESSION) {
                ...
                $GLOBALS['_CRM_CORE_SELECTOR_CONTROLLER']['_template']->assign_by_ref( "{$this->_prefix}pager"  , $this->_pager   );
                $GLOBALS['_CRM_CORE_SELECTOR_CONTROLLER']['_template']->assign_by_ref( "{$this->_prefix}sort"   , $this->_sort    );

                $GLOBALS['_CRM_CORE_SELECTOR_CONTROLLER']['_template']->assign_by_ref( "{$this->_prefix}columnHeaders", $columnHeaders );
                $GLOBALS['_CRM_CORE_SELECTOR_CONTROLLER']['_template']->assign_by_ref( "{$this->_prefix}rows"         , $rows          );
                $GLOBALS['_CRM_CORE_SELECTOR_CONTROLLER']['_template']->assign       ( "{$this->_prefix}rowsEmpty"    , $rowsEmpty     );
                $GLOBALS['_CRM_CORE_SELECTOR_CONTROLLER']['_template']->assign       ( "{$this->_prefix}qill"         , $qill          );
            }


            // always store the current pageID and sortID
            $this->_store->set( $this->_prefix . CRM_UTILS_PAGER_PAGE_ID      ,
                                $this->_pager->getCurrentPageID       ( ) );
            $this->_store->set( $this->_prefix . CRM_UTILS_SORT_SORT_ID       ,
                                $this->_sort->getCurrentSortID        ( ) );
            $this->_store->set( $this->_prefix . CRM_UTILS_SORT_SORT_DIRECTION,
                                $this->_sort->getCurrentSortDirection ( ) );
            $this->_store->set( $this->_prefix . CRM_UTILS_SORT_SORT_ORDER    ,
                                $this->_sort->orderBy                 ( ) );
            $this->_store->set( $this->_prefix . CRM_UTILS_PAGER_PAGE_ROWCOUNT,
     ...
     */
     function getRows( $form ) {
        if ($form->_output == CRM_CORE_SELECTOR_CONTROLLER_EXPORT|| $form->_output == CRM_CORE_SELECTOR_CONTROLLER_SCREEN) {
            //get rows (without paging criteria)
    ...
    function moveFromSessionToTemplate()
    {
        ...
        if ( $rows ) {
            $GLOBALS['_CRM_CORE_SELECTOR_CONTROLLER']['_template']->assign( "{$this->_prefix}aToZ"  ,
                                      $this->_store->get( "{$this->_prefix}AToZBar" ) );
        }

        $GLOBALS['_CRM_CORE_SELECTOR_CONTROLLER']['_template']->assign_by_ref( "{$this->_prefix}sort"  , $this->_sort    );
        $GLOBALS['_CRM_CORE_SELECTOR_CONTROLLER']['_template']->assign( "{$this->_prefix}columnHeaders", $this->_store->get( "{$this->_prefix}columnHeaders" ) );
        $GLOBALS['_CRM_CORE_SELECTOR_CONTROLLER']['_template']->assign( "{$this->_prefix}rows"         , $rows                                 );
        $GLOBALS['_CRM_CORE_SELECTOR_CONTROLLER']['_template']->assign( "{$this->_prefix}rowsEmpty"    , $this->_store->get( "{$this->_prefix}rowsEmpty" )     );
        $GLOBALS['_CRM_CORE_SELECTOR_CONTROLLER']['_template']->assign( "{$this->_prefix}qill"         , $this->_store->get( "{$this->_prefix}qill" )          );
        ...
        if ( $this->_print ) {
            $content = $GLOBALS['_CRM_CORE_SELECTOR_CONTROLLER']['_template']->fetch( 'CRM/common/print.tpl' );
        } else {
