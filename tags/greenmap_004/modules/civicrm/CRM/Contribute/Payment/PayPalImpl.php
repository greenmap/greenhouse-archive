...
define( 'CRM_CONTRIBUTE_PAYMENT_PAYPALIMPL_CHARSET','iso-8859-1');
$GLOBALS['_CRM_CONTRIBUTE_PAYMENT_PAYPALIMPL']['_singleton'] =  null;
...
class CRM_Contribute_Payment_PayPalImpl extends CRM_Contribute_Payment {
     ...
     * @return void 
     */ 
    function CRM_Contribute_Payment_PayPalImpl( $mode ) {
        ...
        if ( $config->paymentUsername[$mode] ) {
            ...
            if ( PayPal::isError( $handler ) ) {
                return CRM_Contribute_Payment_PayPalImpl::error( $handler );
            }
            ...
            if ( PayPal::isError( $this->_profile ) ) {
                return CRM_Contribute_Payment_PayPalImpl::error( $this->_profile );
            }
            ...
            $this->_handler =& ProfileHandler_File::getInstance( array(
                                                                       ...
                                                                       'charset' => CRM_CONTRIBUTE_PAYMENT_PAYPALIMPL_CHARSET,
                                                                       )
                                                                 );
        ...
        if ( PayPal::isError( $this->_caller ) ) {
            $ret = CRM_Contribute_Payment_PayPalImpl::error( $this->_caller );
            $this->_caller = null;
            return $ret;
        }
    }
     ...
     */ 
     function &singleton( $mode ) {
        if ($GLOBALS['_CRM_CONTRIBUTE_PAYMENT_PAYPALIMPL']['_singleton'] === null ) { 
            $GLOBALS['_CRM_CONTRIBUTE_PAYMENT_PAYPALIMPL']['_singleton'] =& new CRM_Contribute_Payment_PaypalImpl( $mode );
        } 
        return $GLOBALS['_CRM_CONTRIBUTE_PAYMENT_PAYPALIMPL']['_singleton']; 
    } 
    ...
    function setExpressCheckOut( &$params ) {
        if ( ! $this->_caller ) {
            return CRM_Contribute_Payment_PayPalImpl::error( );
        }

        $orderTotal =& PayPal::getType( 'BasicAmountType' );

        if ( PayPal::isError( $orderTotal ) ) {
            return CRM_Contribute_Payment_PayPalImpl::error( $orderTotal );
        }

        $orderTotal->setattr('currencyID', $params['currencyID'] );
        $orderTotal->setval( $params['amount'], CRM_CONTRIBUTE_PAYMENT_PAYPALIMPL_CHARSET);
        ...
        if ( PayPal::isError( $setExpressCheckoutRequestDetails ) ) {
            return CRM_Contribute_Payment_PayPalImpl::error( $setExpressCheckoutRequestDetails );
        }

        $setExpressCheckoutRequestDetails->setCancelURL ( $params['cancelURL'], CRM_CONTRIBUTE_PAYMENT_PAYPALIMPL_CHARSET);
        $setExpressCheckoutRequestDetails->setReturnURL ( $params['returnURL'], CRM_CONTRIBUTE_PAYMENT_PAYPALIMPL_CHARSET);
        $setExpressCheckoutRequestDetails->setInvoiceID ( $params['invoiceID'], CRM_CONTRIBUTE_PAYMENT_PAYPALIMPL_CHARSET);
        ...
        if ( PayPal::isError( $setExpressCheckout ) ) {
            return CRM_Contribute_Payment_PayPalImpl::error( $setExpressCheckout );
        }
        ...
        if (PayPal::isError( $result  ) ) { 
            return CRM_Contribute_Payment_PayPalImpl::error( $result );
        }

        $result =& CRM_Contribute_Payment_PayPalImpl::checkResult( $result );
    ...
    function getExpressCheckoutDetails( $token ) {
        ...
        if ( PayPal::isError( $getExpressCheckoutDetails ) ) {
            return CRM_Contribute_Payment_PayPalImpl::error( $getExpressCheckoutDetails );
        }

        $getExpressCheckoutDetails->setToken( $token, CRM_CONTRIBUTE_PAYMENT_PAYPALIMPL_CHARSET);
    ...
    function doExpressCheckout( &$params ) {
        ...
        if ( PayPal::isError( $paymentDetails ) ) {
            return CRM_Contribute_Payment_PayPalImpl::error( $paymentDetails );
        }

        $paymentDetails->setOrderTotal( $orderTotal );
        $paymentDetails->setInvoiceID( $params['invoiceID'], CRM_CONTRIBUTE_PAYMENT_PAYPALIMPL_CHARSET);
        ...
        if ( PayPal::isError( $doExpressCheckoutPaymentRequestDetails ) ) {
            return CRM_Contribute_Payment_PayPalImpl::error( $doExpressCheckoutPaymentRequestDetails );
        }

        $doExpressCheckoutPaymentRequestDetails->setPaymentDetails( $paymentDetails );
        $doExpressCheckoutPaymentRequestDetails->setPayerID       ( $params['payer_id']      , CRM_CONTRIBUTE_PAYMENT_PAYPALIMPL_CHARSET);
        $doExpressCheckoutPaymentRequestDetails->setToken         ( $params['token']         , CRM_CONTRIBUTE_PAYMENT_PAYPALIMPL_CHARSET);
        $doExpressCheckoutPaymentRequestDetails->setPaymentAction ( $params['payment_action'], CRM_CONTRIBUTE_PAYMENT_PAYPALIMPL_CHARSET);
        $doExpressCheckoutPayment =& PayPal::getType( 'DoExpressCheckoutPaymentRequestType' );

        if ( PayPal::isError( $doExpressCheckoutPayment ) ) {
            return CRM_Contribute_Payment_PayPalImpl::error( $doExpressCheckoutPayment );
        }
        ...
        $params['gross_amount'  ] = CRM_Contribute_Payment_PayPalImpl::getAmount( $paymentInfo->GrossAmount );
        $params['fee_amount'    ] = CRM_Contribute_Payment_PayPalImpl::getAmount( $paymentInfo->FeeAmount    );
        $params['net_amount'    ] = CRM_Contribute_Payment_PayPalImpl::getAmount( $paymentInfo->SettleAmount );
    ...
    function doDirectPayment( &$params ) {
        ...
        if ( PayPal::isError( $payerName ) ) {
            return CRM_Contribute_Payment_PayPalImpl::error( $payerName );
        }

        $payerName->setLastName  ( $params['last_name'  ], CRM_CONTRIBUTE_PAYMENT_PAYPALIMPL_CHARSET);
        $payerName->setMiddleName( $params['middle_name'], CRM_CONTRIBUTE_PAYMENT_PAYPALIMPL_CHARSET);
        $payerName->setFirstName ( $params['first_name' ], CRM_CONTRIBUTE_PAYMENT_PAYPALIMPL_CHARSET);
        $address =& PayPal::getType('AddressType');

        if (PayPal::isError($address)) {
            return CRM_Contribute_Payment_PayPalImpl::error( $address );
        }

        $address->setStreet1        ( $params['street_address'], CRM_CONTRIBUTE_PAYMENT_PAYPALIMPL_CHARSET);
        $address->setCityName       ( $params['city']          , CRM_CONTRIBUTE_PAYMENT_PAYPALIMPL_CHARSET);
        $address->setStateOrProvince( $params['state_province'], CRM_CONTRIBUTE_PAYMENT_PAYPALIMPL_CHARSET);
        $address->setPostalCode     ( $params['postal_code']   , CRM_CONTRIBUTE_PAYMENT_PAYPALIMPL_CHARSET);
        $address->setCountry        ( $params['country']       , CRM_CONTRIBUTE_PAYMENT_PAYPALIMPL_CHARSET);
        $cardOwner =& PayPal::getType( 'PayerInfoType' );

        if ( PayPal::isError( $cardOwner ) ) {
            return CRM_Contribute_Payment_PayPalImpl::error( $cardOwner );
        }

        $cardOwner->setPayer( $params['email'] );
        $cardOwner->setAddress( $address );
        $cardOwner->setPayerCountry( $params['country'], CRM_CONTRIBUTE_PAYMENT_PAYPALIMPL_CHARSET);
        $cardOwner->setPayerName( $payerName );
        $creditCard =& PayPal::getType( 'CreditCardDetailsType' );

        if ( PayPal::isError( $creditCard ) ) {
            return CRM_Contribute_Payment_PayPalImpl::error( $creditCard );
        }

        $creditCard->setCardOwner( $cardOwner );
        $creditCard->setCVV2            ( $params['cvv2']              , CRM_CONTRIBUTE_PAYMENT_PAYPALIMPL_CHARSET);
        $creditCard->setExpYear         ( $params['year' ]             , CRM_CONTRIBUTE_PAYMENT_PAYPALIMPL_CHARSET);
        $creditCard->setExpMonth        ( $params['month']             , CRM_CONTRIBUTE_PAYMENT_PAYPALIMPL_CHARSET);
        $creditCard->setCreditCardNumber( $params['credit_card_number'], CRM_CONTRIBUTE_PAYMENT_PAYPALIMPL_CHARSET);
        $creditCard->setCreditCardType  ( $params['credit_card_type']  , CRM_CONTRIBUTE_PAYMENT_PAYPALIMPL_CHARSET);
        ...
        if ( PayPal::isError( $doDirectPaymentRequestDetails ) ) {
            return CRM_Contribute_Payment_PayPalImpl::error( $doDirectPaymentRequestDetails );
        }

        $doDirectPaymentRequestDetails->setCreditCard    ( $creditCard     );
        $doDirectPaymentRequestDetails->setPaymentDetails( $paymentDetails );
        $doDirectPaymentRequestDetails->setIPAddress     ( $params['ip_address'    ], CRM_CONTRIBUTE_PAYMENT_PAYPALIMPL_CHARSET);
        $doDirectPaymentRequestDetails->setPaymentAction ( $params['payment_action'], CRM_CONTRIBUTE_PAYMENT_PAYPALIMPL_CHARSET);
        $doDirectPayment =& PayPal::getType( 'DoDirectPaymentRequestType' );

        if ( PayPal::isError( $doDirectPayment ) ) {
            return CRM_Contribute_Payment_PayPalImpl::error( $doDirectPayment );
        }
        ...
        $params['gross_amount'  ] = CRM_Contribute_Payment_PayPalImpl::getAmount( $result->Amount );
        return $params;
    }
