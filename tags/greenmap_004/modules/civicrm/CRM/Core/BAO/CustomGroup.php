...
class CRM_Core_BAO_CustomGroup extends CRM_Core_DAO_CustomGroup {
      ...
      function addMenuTabs($entityType, $path, $startWeight)
    {
        ...
        while($customGroupDAO->fetch()) {
            ...
            $menu['type']    = CRM_CORE_MENU_CALLBACK;
            $menu['crmType'] = CRM_CORE_MENU_LOCAL_TASK;
            ...
            foreach ($group['fields'] as $field) {
                ...
                case 'CheckBox':
                    if ($viewMode) {
                        ...
                        $checkedData = explode(CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR, substr($value,1,-1));
                        ...
                        if (isset($field['customValue']['data'])) {
                            $checkedData = explode(CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR,substr($field['customValue']['data'],1,-1));
                            foreach($customOption as $val) {
                                if (in_array($val['value'], $checkedData)) {
                                    $defaults[$elementName][$val['value']] = 1;
                                } else {
                            ...
                            $checkedValue = explode(CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR, substr($value,1,-1));
                            foreach($customOption as $val) {
        ...
        foreach ($groupTree as $group) {
            foreach ($group['fields'] as $field) {
                ...
                case 'CheckBox':  
                    ...
                    if ( ! empty( $v ) ) {
                        ...
                        $groupTree[$groupId]['fields'][$fieldId]['customValue']['data'] = 
                            CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR.implode(CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR, $customValue).CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR;
                    } else {
                        $groupTree[$groupId]['fields'][$fieldId]['customValue']['data'] = null;
                    }
                    break;
                ...
                case 'Multi-Select':  
                    ...
                    if ( ! empty( $v ) ) {
                        $groupTree[$groupId]['fields'][$fieldId]['customValue']['data'] = 
                            CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR.implode(CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR, $v).CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR;
                    } else {
                        $groupTree[$groupId]['fields'][$fieldId]['customValue']['data'] = null;
                    }
                    break;
