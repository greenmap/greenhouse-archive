     ...
     */
     function add(&$params, &$ids) {
        ...
        if(is_array($prefComm)) {
            $prefComm = implode(CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR,array_keys($prefComm));
     ...
     */
     function &getValues( &$params, &$values, &$ids ) {
        ...
        if ( $contact->find(true) ) {
            ...
            $comm =explode(CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR,$contact->preferred_communication_method);
...
WHERE civicrm_contact.id IN $idString AND civicrm_address.geo_code_1 is not null AND civicrm_address.geo_code_2 is not null";
     ...
     * @access public
     */
     function getNumOpenActivity( $id, $admin = false ) {
     ...
     * @static
     */
     function &getOpenActivities(&$params, $offset=null, $rowCount=null, $sort=null, $type='Activity', $admin = false) {
        $dao =& new CRM_Core_DAO();
        if ( $admin ) {
            $clause = null;
            $params = array( );
...
";
     ...
     */
     function getHierContactDetails( $contactId, &$fields ) {
        ...
        $returnProperties =& CRM_Contact_BAO_Contact::makeHierReturnProperties( $fields, $contactId );
     ...
     * @access public
     */
     function createProfileContact( &$params, &$fields, $contactID = null, $addToGroupID = null, $ufGroupId = null ) {
        require_once 'CRM/Utils/Hook.php';
        if ( $contactID ) {
                ...
                if ($key === 'individual_suffix') { 
                    ...
                    if ($customFieldID = CRM_Core_BAO_CustomField::getKeyID($key)) {
                        ...
                        if ( $customFields[$customFieldID][3] == 'CheckBox' ) {
                            $value = CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR.implode(CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR, array_keys($value)).CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR;
                        } 
                        
                        if ( $customFields[$customFieldID][3] == 'Multi-Select' ) {
                            $value = CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR.implode(CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR, $value).CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR;
                        }
                        // fix the date field 
...
//         exit();
        ...
        if ( CRM_Core_Permission::access( 'Quest' ) && $studentFieldPresent ) {
            ...
            foreach( $fields as $field ) {
                if ( $params[$field] ) {
                    $params[$field] = implode(CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR,array_keys($params[$field]));
                }
            }
     ...
     * @static
     */
     function check_contact_exists($contactId)
    {
       require_once "CRM/Contact/DAO/Contact.php";
       $contact =& new CRM_Contact_DAO_Contact();
     ...
     function &matchContactOnEmail( $mail ) {
       $query = "
