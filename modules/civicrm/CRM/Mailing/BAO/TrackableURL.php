...
$GLOBALS['_CRM_MAILING_BAO_TRACKABLEURL']['urlCache'] =  array();
...
class CRM_Mailing_BAO_TrackableURL extends CRM_Mailing_DAO_TrackableURL {
     ...
     */
      function getTrackerURL($url, $mailing_id, $queue_id) {
        ...
        if (array_key_exists($url, $GLOBALS['_CRM_MAILING_BAO_TRACKABLEURL']['urlCache'])) {
            return $GLOBALS['_CRM_MAILING_BAO_TRACKABLEURL']['urlCache'][$url];
        }
        ...
        if (preg_match('/\.(png|jpg|jpeg|gif)$/i', $url)
            ...
            $GLOBALS['_CRM_MAILING_BAO_TRACKABLEURL']['urlCache'][$url] = $url;
        } else {
            ...
            $redirect = $config->userFrameworkResourceURL . 'extern/url.php?q=' . $queue_id .
                        '&u=' . $id;
            $GLOBALS['_CRM_MAILING_BAO_TRACKABLEURL']['urlCache'][$url] = $redirect;
            $tracker->free();
        }
