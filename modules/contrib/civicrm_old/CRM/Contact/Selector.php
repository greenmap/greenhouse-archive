 ...
 * $Id: Selector.php 5959 2006-07-05 23:00:00Z lobo $
 *
 */

$GLOBALS['_CRM_CONTACT_SELECTOR']['_links'] =  null;
$GLOBALS['_CRM_CONTACT_SELECTOR']['_columnHeaders'] = null;
$GLOBALS['_CRM_CONTACT_SELECTOR']['_properties'] =  array('contact_id', 'contact_type', 'contact_sub_type', 
                                'sort_name', 'street_address',
                                'city', 'state_province', 'postal_code', 'country',
                                'geo_code_1', 'geo_code_2',
                                'email', 'phone', 'status' );
$GLOBALS['_CRM_CONTACT_SELECTOR']['skipFields'] =  array( 'group', 'tag' );
...
class CRM_Contact_Selector extends CRM_Core_Selector_Base {
     ...
     * @access public
     */
    function CRM_Contact_Selector(&$formValues, &$params, &$returnProperties, $action = CRM_CORE_ACTION_NONE) 
    {
    ...
    function &getColumnHeaders($action = null, $output = null) 
    {
        ...
        if ( $output == CRM_CORE_SELECTOR_CONTROLLER_EXPORT || $output == CRM_CORE_SELECTOR_CONTROLLER_SCREEN ) {
            ...
            $empty = true;
            if ( ! $GLOBALS['_CRM_CONTACT_SELECTOR']['_columnHeaders'] ) {
                $GLOBALS['_CRM_CONTACT_SELECTOR']['_columnHeaders'] = array( array( 'name' => '' ),
                                               array(
                                                     'name'      => ts('Name'),
                                                     'sort'      => 'sort_name',
                                                     'direction' => CRM_UTILS_SORT_ASCENDING,
                ...
                foreach ( $this->_fields as $name => $field ) { 
                    if ( $field['in_selector'] &&
                         ! in_array( $name, $GLOBALS['_CRM_CONTACT_SELECTOR']['skipFields'] ) ) {
                        $GLOBALS['_CRM_CONTACT_SELECTOR']['_columnHeaders'][] = array( 'name'      => $field['title'],
                                                         'sort'      => $name,
                                                         'direction' => $direction );
                ...
                if ( $empty ) {
                    ...
                    $GLOBALS['_CRM_CONTACT_SELECTOR']['_columnHeaders'][] = array('desc' => ts('Actions'));
                }
            }
            ...
            $properties =& CRM_Contact_Selector::makeProperties( $this->_returnProperties );
            ...
            foreach ( $properties as $prop ) {
                ...
                $GLOBALS['_CRM_CONTACT_SELECTOR']['_columnHeaders'][] = array( 'name' => $title, 'desc' => $prop );
            }
            $GLOBALS['_CRM_CONTACT_SELECTOR']['_columnHeaders'][] = array('desc' => ts('Actions'));
            return $GLOBALS['_CRM_CONTACT_SELECTOR']['_columnHeaders'];
        } else {
    ...
    function &getRows($action, $offset, $rowCount, $sort, $output = null) {
        ...
        if ($this->_ufGroupID ) {
            ...
            foreach ( $this->_fields as $key => $field ) {
                if ( $field['in_selector'] && 
                     ! in_array( $key, $GLOBALS['_CRM_CONTACT_SELECTOR']['skipFields'] ) ) { 
                    if ( strpos( $key, '-' ) !== false ) {
            ...
            $names =& CRM_Contact_Selector::makeProperties( $this->_returnProperties );
        } else {
            $names = $GLOBALS['_CRM_CONTACT_SELECTOR']['_properties'];
        }
        ...
        if ( CRM_Core_Permission::access( 'Quest' ) ) {
            ...
            $multipleSelectFields = $GLOBALS['_CRM_QUEST_BAO_STUDENT']['multipleSelectFields'];
        }
