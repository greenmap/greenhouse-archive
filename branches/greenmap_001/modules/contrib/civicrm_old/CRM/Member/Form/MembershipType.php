...
define( 'CRM_MEMBER_FORM_MEMBERSHIPTYPE_MAX_CONTACTS',50);
...
class CRM_Member_Form_MembershipType extends CRM_Member_Form
{
     ...
     function buildQuickForm( ) 
    {
        ...
        } elseif ( $this->_action & CRM_CORE_ACTION_UPDATE ) {
            $searchBtn = ts('Change');
        } else {
            $searchBtn = ts('Search');
        }
     ...
     */
     function formRule( &$params ) {
        ...
        if ( !$params['_qf_MembershipType_refresh'] ) {
            ...
            if ( !$params['contact_check'] && $params['action']!= CRM_CORE_ACTION_UPDATE ) {
    ...
    function search(&$params) {
        ...
        if ( $searchCount <= CRM_MEMBER_FORM_MEMBERSHIPTYPE_MAX_CONTACTS) {
            // get the result of the search
            $result = $query->searchQuery(0, CRM_MEMBER_FORM_MEMBERSHIPTYPE_MAX_CONTACTS, null);
